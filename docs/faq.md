...
| Menu bar % has turned red | That window is at 90% or more of its limit — the same colour, at the same threshold, as its row in the popover. Measured on real usage, 90% flagged every window that went on to run out, about a quarter of an hour ahead (which is as early as the source allows; see docs/design-budgets-and-alerts.md). Turn it off under **Menu Bar Shows → Colour the Limit at 90%**. |
| `make-app.sh` fails with *plugin for module 'SwiftUIMacros' not found* | The build is using the Command Line Tools, which lack SwiftUI's macros on macOS 27. Run `sudo xcode-select -s /Applications/Xcode.app` then `sudo xcodebuild -license accept`. The script prints this too. |
